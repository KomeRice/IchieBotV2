...
{
    public class DressLegacyEmbedHelper
    {
        // TODO: Refine async scheme
        private readonly DatabaseLegacyService _db;
        private readonly RankingLegacyService _rankingLegacy;
        ...
        public DressLegacyEmbedHelper(DatabaseLegacyService db, RankingLegacyService rankingLegacy)
        {
            _db = db;
        ...
        public async Task<Embed> LegacyToEmbedOverview(StageGirl dress, int rb = 0)
        {
            ...
            var ranks = _rankingLegacy.GetRanks(dress.DressId[2..], rb);
            ...
            var embedFieldBuilders = new List<EmbedFieldBuilder>()
            {
                new EmbedFieldBuilder()
                {
                    ...
                    Value = "```" + "Power Score".PadRight(18) + $"{stats[0]}".PadRight(10) +
                            $"[#{ranks[0]}/{_rankingLegacy.GetMax(RankingLegacyService.Parameter.PowerScore, rb)}]\n" +
                            "MaxHP".PadRight(18) + $"{stats[1]}".PadRight(10) +
                            $"[#{ranks[1]}/{_rankingLegacy.GetMax(RankingLegacyService.Parameter.MaxHp, rb)}]\n" +
                            "ACT Power".PadRight(18) + $"{stats[2]}".PadRight(10) +
                            $"[#{ranks[2]}/{_rankingLegacy.GetMax(RankingLegacyService.Parameter.Act, rb)}]\n" +
                            "NormDef".PadRight(18) + $"{stats[3]}".PadRight(10) +
                            $"[#{ranks[3]}/{_rankingLegacy.GetMax(RankingLegacyService.Parameter.NormDef, rb)}]\n" +
                            "SpDef".PadRight(18) + $"{stats[4]}".PadRight(10) +
                            $"[#{ranks[4]}/{_rankingLegacy.GetMax(RankingLegacyService.Parameter.SpDef, rb)}]\n" +
                            "Agility".PadRight(18) + $"{stats[5]}".PadRight(10) +
                            $"[#{ranks[5]}/{_rankingLegacy.GetMax(RankingLegacyService.Parameter.Agility, rb)}]\n" +
                            "Row Position".PadRight(18) + _rankingLegacy.GetPositionIndex(dress.DressId[2..]) + "\n" +
